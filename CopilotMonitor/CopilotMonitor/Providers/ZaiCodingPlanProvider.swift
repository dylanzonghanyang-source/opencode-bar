...
struct ZaiQuotaLimitItem: Decodable {
    ...
    let usage: Int?
    let remaining: Int?
    /// Window unit/length used to distinguish the plan's rolling windows:
...
final class ZaiCodingPlanProvider: ProviderProtocol {
    ...
    func fetch() async throws -> ProviderResult {
        ...
        let creditSessionLimit = tokenLimit == nil && mcpLimit == nil
            ? creditLimits.first { $0.unit == 3 }
            : nil
        let creditWeeklyLimit = tokenLimit == nil && mcpLimit == nil
            ? creditLimits.first { $0.unit == 6 }
            : nil
